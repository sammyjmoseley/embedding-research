...
class NoEmbeddingClassifier:
    def __init__(self):
        with tf.variable_scope('classifier'):
            ...
            x = tf.placeholder_with_default(x, x.shape)
            self.embedding = x

            for layer in classification_layer:
                name, (dim, out) = layer
                x = create_fc_layer(self, name, x, dim, out)
                dim = out
            ...
            with tf.variable_scope('class_loss'):
                ...
                self.class_loss = tf.nn.softmax_cross_entropy_with_logits(labels=self.y_, logits=self.before_softmax)
    ...
    def __init__(self, no_embedding_classifier):
        ...
        with tf.variable_scope('rotated_embedding'):
            ...
            with tf.variable_scope('input'):
                ...
                self.y_ = tf.placeholder(tf.float32, shape=[None, 10], name='y_')
                self.keep_prob = no_embedding_classifier.keep_prob
            dim = 1
            x = self.x
            x = tf.reshape(x, (-1, 784))
            ...
            with tf.variable_scope('read_out'):
                non_rotated_embedding = self.no_embedding_classifier.embedding
                embedding_dist = compute_euclidean_distances(non_rotated_embedding, self.embedding)
